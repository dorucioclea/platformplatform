...
| ScrollArea                | shadcn `scroll-area` | Use inside DialogBody for intentionally scrollable content                                                                                                   |
...
2. **Apple HIG control heights**: `h-[var(--control-height)]` (44px desktop / 38px tablet) and the `--control-height-sm` / `--control-height-xs` variants. Smaller controls add a 44px tap target via `after:absolute after:-inset-3`.
